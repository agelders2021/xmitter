...
//  carrier that feeds the keyer's pad/LPF/buffer chain.  CLK2 only — CLK0 and
//  CLK1 are left disabled (Rev A analog board routes CLK2 to the buffer input).
//
//  Design references:
...
// into a known state, and disables all outputs.  CLK2 stays off until
// vfo_on() is called.  Idempotent — safe to call again to recover.
esp_err_t init(i2c_master_bus_handle_t bus);

// Set CLK2 to f_hz.  Range: 8 kHz – 160 MHz (Si5351 spec); xmitter only ever
// needs 14.0–14.4 MHz so we don't bother with the multi-band fractional cases.
// Programmes PLLA + Multisynth2 from the requested frequency.  Output state
// (on/off) is unchanged — call vfo_on() once you want RF.
esp_err_t set_freq(uint32_t f_hz);

// Enable / disable CLK2 output (drive strength stays at 8 mA — see datasheet
...
// True if the driver believes CLK2 is currently on.
bool is_on();
