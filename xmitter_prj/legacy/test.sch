...
<Properties>
  <View=57,-69,1939,896,1.48069,367,228>
  <Grid=10,10,1>
  <DataSet=test.dat>
...
<Components>
  ...
  <R R_CAR_BIAS 1 710 490 15 -26 0 1 "100 kOhm" 1 "26.85" 0 "0.0" 0 "0.0" 0 "26.85" 0 "US" 0>
  ...
  <VProbe Pr1 1 1020 360 28 -31 0 0>
...
<Wires>
  ...
  <700 460 700 470 "" 0 0 0 "">
  <700 460 710 460 "" 0 0 0 "">
  ...
  <1050 380 1050 620 "" 0 0 0 "">
  <750 620 1050 620 "" 0 0 0 "">
  <1010 380 1010 410 "" 0 0 0 "">
  <1010 410 750 410 "" 0 0 0 "">
</Wires>
<Diagrams>
  <Rect 1010 270 240 160 3 #c0c0c0 1 00 1 0 0.2 1 1 -0.1 0.5 1.1 1 -0.1 0.5 1.1 315 0 225 1 0 0 "" "" "">
	<"ngspice/tran.v(pr1)" #0000ff 1 3 0 0 0>
  </Rect>
</Diagrams>
<Paintings>
  <Text 100 680 10 #000000 0 "MC1496 VCA test, 14.175 MHz 20m CW TX. SpLib X4 (MC1496_MODEL.cir) at cx=610 cy=470; odd pins x=580, even x=640, 60-unit y-spacing. P1=SIG_P@y290 P2=GAIN_A@y290 P3=GAIN_B@y350 P4=SIG_N@y350 P5=BIAS@y410 P6=OUT_P@y410 P8=CAR_P@y470 P10=CAR_N@y530 P12=OUT_N@y590 P14=VEE@y650. Supply: V1=+12V V2=8.2V in series, GND between them: VDD=+12V VEE=-8.2V. Bias: R_BIAS=6.8k (P5-GND) sets tail current; R_RE=1k (P2-P3) sets gain; R_SIG_P=10k (P1-GND). AGC divider: R_SCALE1=100k + R_SCALE2=5.1k (V_AGC to P4). Carrier: V_RF=0.378V 14.175MHz into CC_CAR=10nF to P8; R_CAR_BIAS=100k (P8 node to GND) for DC return; P10 direct-GND (CCAR_N removed). Loads: R_LOAD_P=R_LOAD_N=3.9k (VDD to P6 and P12). VProbe Pr1 = v(OUT_P)-v(OUT_N).">
  <Text 100 710 10 #000000 0 "Results 2026-06-06 (pre-fix run): AGC=0V ~0.69V p-p (feedthrough); AGC=10V ~1.61V p-p (+1.03V/-0.58V) — asymmetric. Root cause: CC_CAR had no DC return on P8 side; asymmetric Q3/Q6 base currents charged CC_CAR during sim startup and shifted P8 DC level, biasing the upper quad off-center. Fix applied: R_CAR_BIAS=100k from P8 node to GND + P10 direct-grounded. Both carrier port pins now at 0V DC at quiescence so upper quad is symmetric. Re-simulate to confirm. Secondary: P1 biased through 10k, P4 through 4.85k to AGC — minor signal-port imbalance; for perfect null use equal resistors both sides.">
</Paintings>
